...
if(rolesList&&typeof chapterRoles!=='undefined')rolesList.innerHTML=chapterRoles.map((role,index)=>`<article class="role${role.image?' has-photo':''} reveal"><span>${String(index+1).padStart(2,'0')}</span>${role.image?`<img src="${role.image}" alt="${role.name}, ${role.title}" loading="lazy">`:''}<div><h3>${role.title}</h3><strong>${role.name}</strong></div><p>${role.description}</p></article>`).join('');
